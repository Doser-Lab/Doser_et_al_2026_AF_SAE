# 02b-determine-factors-stage-2.R: preliminary script to determine how 
#                                  many factors are needed for the second 
#                                  stage of the two stage model. 
rm(list = ls())
library(spAbundance)

# Directories -------------------------------------------------------------
# Determine what machine you're on and change directories as needed.
machine.name <- Sys.info()['nodename']
if (machine.name == 'pop-os') {
  out.dir <- 'results/'
  data.dir <- 'data/'
} else { # Running on NCSU machine
  out.dir <- '/share/doserlab/jwdoser/DSKW25/results/'
  data.dir <- '/share/doserlab/jwdoser/DSKW25/data/'
}

# Read in the data set ----------------------------------------------------
load(paste0(data.dir, 'spAbundance_data.rda'))

# Specify model inputs ----------------------------------------------------
# Priors ------------------------------
dist.mat <- dist(data.list.2$coords)
# The average distance between provinces in the study area.
high.dist <- 152116.2
# Setting to 1km to avoid overfitting.
low.dist <- 1000
prior.list <- list(beta.comm.normal = list(mean = 0, var = 10),
                   tau.sq.beta.ig = list(a = 0.1, b = 0.1), 
                   tau.sq.ig = list(a = 0.001, b = 0.001),
                   phi.unif = list(3 / high.dist, 3 / low.dist))

# Starting values
inits.list <- list(beta.comm = 0, beta = 0, tau.sq.beta = 1,
                   phi = 3 / mean(c(high.dist, low.dist)), fix = TRUE)
# Tuning
tuning.list <- list(phi = 0.75)

# MCMC criteria
n.batch <- 1000
batch.length <- 25
n.burn <- 10000
n.thin <- 5
n.chains <- 1

# Number of latent factors
n.factors <- 1:8

# Fit the model -----------------------------------------------------------
# Multiple models are fit via the loop.
for (i in n.factors) {
  print(paste0("Currently on factor ", i, " out of ", length(n.factors)))
  out <- sfMsAbund(formula = ~ scale(Elevation) + I(scale(Elevation)^2) +
                               scale(ppt) + I(scale(ppt)^2) + (1 | Province_num),
                 data = data.list.2,
                 n.batch = n.batch,
                 batch.length = batch.length,
                 inits = inits.list,
                 family = 'zi-Gaussian',
                 tuning = tuning.list,
                 n.neighbors = 10,
                 cov.model = 'exponential',
                 priors = prior.list,
                 n.factors = i,
                 accept.rate = 0.43,
                 n.omp.threads = 1,
                 verbose = TRUE,
                 n.report = 10,
                 n.burn = n.burn,
                 n.thin = n.thin,
                 n.chains = n.chains)
  
  # Save the results to a hard drive ----------------------------------------
  waic.out <- waicAbund(out, by.sp = TRUE)
  save(waic.out, file = paste0(out.dir, 'prelim-stage-2-waic-', i, '-factors-',
                               Sys.Date(), '.rda'))
  rm(out)
  gc()
}

